...
export function registerLocationHandlers(io: Server): void {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("track:subscribe", (data: TrackSubscribe) => {
      if (payload.role !== "Student" && payload.role !== "Vendor") {
        socket.emit("error", { message: "Only Students and Vendors can subscribe to order tracking" });
        return;
      }
      ...
      console.log(`[Socket] ${payload.role} ${payload.sub} subscribed to room: ${room}`);
    });
